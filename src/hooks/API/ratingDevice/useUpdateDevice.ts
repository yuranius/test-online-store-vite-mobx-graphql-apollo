...
export function useUpdateDevice() {
  ...
  const [getRatingDevice, { error: errorGetRatingDevice }] =
    useLazyQuery(GET_RATING_DEVICE);
  ...
  const updateDevice = async (id: string | undefined) => {
    ...
    const sumRate = dataGetRating.ratings.edges
      .map(({ node }: { node: { rate: number } }) => node.rate)
    ...
    let { data: totalRate } = await update({
      ...
      update(cache, { data: { updateDevice } }) {
        ...
        Promise.allSettled([
          ...
          res.map(({ status, value }: any, index: number) => {
            let newDevice = [];
            let data = {};

            if (!!res[index].value?.devices) {
              newDevice = res[index].value.devices.edges.filter(
                ({ node }: any) => node.objectId === id
              );
              data = {
                devices: {
                  ...
                  ...{
                    edges: [
                      {
                        ...newDevice[0],
                        node: { ...newDevice[0].node, rating: rate },
                      },
                      ...res[index].value.devices.edges.filter(
                        ({ node }: any) => node.objectId !== id
                      ),
                    ],
                  },
